    ...
    <div class="w-full p-6">
        ...
        <div>
            @if(session()->has('message'))
                <div class="p-3 mb-2 bg-green-300 text-green-800 rounded shadow-sm">
                    {{ session('message') }}
                </div>
            @endif
        </div>
        ...
        <section>
            @if($image)
                <img src="{{ $image }}" width="200" class="mb-2" />
            @endif
            <input type="file" id="image" wire:change="$emit('selectFile')">
        </section>

        <form class="my-4 flex items-center" wire:submit.prevent="addComment">
            ...
            <div class="rounded border shadow p-3 my-2">
                <div class="flex justify-between my-2">
                    <div class="flex">
                        ...
                        <p class="mx-3 py-1 text-xs text-gray-500">{{ $comment->created_at->diffForHumans() }}</p>
                    </div>
                    <i class="fas fa-times text-red-200 hover:text-red-600 cursor-pointer" wire:click="remove({{ $comment->id }})"></i>
                </div>
                <p class="text-gray-800">{{ $comment->body }}</p>
                @if($comment->image)
                    <img src="{{ $comment->imagePath }}" width="200" alt="comment">
                @endif
            </div>
        @endforeach
        ...
        <div class="text-center mt-6">
            {{ $comments->links() }}
{{--            {{ $comments->links('pagination-links-view') }}--}}
        </div>
    </div>
</div>
